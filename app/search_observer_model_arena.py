...
class ModelArenaCase(ArenaModel):
...
ArenaEvaluator = Callable[
    [SearchWaveTelemetry, ResolvedObserverModel],
    Awaitable[SearchObserverRecommendation | None],
]


def validate_replay_case(case: ModelArenaCase) -> None:
    """Reject compact trace-only evidence: Arena requires replay-complete telemetry."""
...
async def evaluate_model_arena_case(
    *,
    case: ModelArenaCase,
    model: ResolvedObserverModel,
    ...
    return observation_from_recommendation(
        ...
        error_code=None if recommendation is not None else "observer_evaluation_failed",
    )
    ...
    for profile_name, items in grouped.items():
        ...
        summaries.append(
            ModelArenaSummary(
                ...
                schema_success_rate=len(valid) / len(configured) if configured else 0.0,
                routing_violation_count=sum(1 for item in items if item.routing_changed),
                mean_latency_ms=mean(item.latency_ms for item in valid) if valid else 0.0,
