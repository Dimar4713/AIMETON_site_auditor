...
def planned_window(content: dict[str, Any]) -> tuple[str, str]:
    ...
    if closed_at:
        ...
        finish = (start_dt + timedelta(days=priority_days(
...
def get_project(token: str, owner: str, number: int) -> dict[str, Any]:
    ...
    for owner_type in ("user", "organization"):
        ...
        query($login:String!, $number:Int!) {{
          {owner_type}(login:$login) {{ projectV2(number:$number) {{ {fields} }} }}
        }}
...
def canonical_fields(project: dict[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    for expected in PLAN_FIELDS:
        matches = [node for node in nodes if node.get("name", "").casefold() == expected.casefold()]
        if len(matches) != 1:
            raise RuntimeError(f"expected exactly one field {expected!r}, found {len(matches)}")
        field = matches[0]
          ...
          node(id:$project) { ... on ProjectV2 {
            items(first:100, after:$cursor) {
              nodes {
                ...
                content {
                  __typename
                  ... on Issue { id number title createdAt closedAt labels(first:20){nodes{name}} repository{nameWithOwner} }
                  ... on PullRequest { id number title createdAt closedAt mergedAt labels(first:20){nodes{name}} repository{nameWithOwner} }
                }
...
def set_date(token: str, project_id: str, item_id: str, field_id: str, value: str) -> None:
    gql(token, """
...
def apply_item(token: str, project: dict[str, Any], fields: dict[str, dict[str, Any]], item: dict[str, Any]) -> dict[str, Any] | None:
    content = item.get("content") or {}
    if not content or (content.get("repository") or {}).get("nameWithOwner") != env("REPOSITORY"):
        return None
    current = existing_dates(item)
    start, finish = planned_window(content)
    desired = {
        "Start date": start,
        "Planned start": start,
        "Target date": finish,
    ...
    conflicts = {name: [current[name], value] for name, value in desired.items() if current.get(name) and current[name] != value}
    if conflicts:
        raise RuntimeError(f"manual/derived plan conflict for #{content.get('number')}: {conflicts}")
    ...
    return {"number": content.get("number"), "kind": content.get("__typename"), "updates": updates, "desired": desired}


def verify(token: str, project_id: str, expected: list[dict[str, Any]]) -> None:
    by_number = {(item.get("content") or {}).get("number"): item for item in list_items(token, project_id)}
    failures = []
    for record in expected:
        item = by_number.get(record["number"])
        actual = existing_dates(item or {})
        if any(actual.get(name) != value for name, value in record["desired"].items()):
            failures.append({"number": record["number"], "expected": record["desired"], "actual": actual})
    if failures:
        raise RuntimeError("planned date read-back failed: " + json.dumps(failures, ensure_ascii=False))


def main() -> int:
    token, owner, number = env("GH_TOKEN"), env("PROJECT_OWNER"), int(env("PROJECT_NUMBER", "0"))
    if not token or not owner or not number or not env("REPOSITORY"):
        raise RuntimeError("GH_TOKEN, PROJECT_OWNER, PROJECT_NUMBER and REPOSITORY are required")
    ...
    print(json.dumps({"project": project["title"], "items_verified": len(records), "updates": records}, ensure_ascii=False))
    return 0
