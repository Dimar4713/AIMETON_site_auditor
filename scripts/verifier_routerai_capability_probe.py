...
# Fixed probe prompts are far smaller than this.  The conservative ceiling is
# used only for an admission preflight; the actual provider usage is recorded
# separately when returned by the API.
PREFLIGHT_INPUT_TOKEN_CEILING = 2048
PREFLIGHT_OUTPUT_TOKEN_CEILING = 4
...
def main() -> int:
    ...
    preflight_cost = _estimated_cost_rub(
        PREFLIGHT_INPUT_TOKEN_CEILING,
        PREFLIGHT_OUTPUT_TOKEN_CEILING,
    )
    ...
    request_payload = build_openai_logprob_probe_payload(model, top_logprobs=20)
    ...
    try:
        ...
        error_payload = {
            ...
            "reason_codes": [f"provider_http_{exc.code}"],
        ...
        _write_result(result_path, error_payload)
        return 2
    ...
    actual_estimated_cost = None
    if prompt_tokens is not None and completion_tokens is not None:
        actual_estimated_cost = _estimated_cost_rub(prompt_tokens, completion_tokens)
    ...
    sanitized = {
        ...
        "max_budget_rub": max_budget_rub,
        "preflight_estimated_cost_rub": round(preflight_cost, 9),
        "actual_estimated_cost_rub": (
            round(actual_estimated_cost, 9)
            if actual_estimated_cost is not None
            else None
        ),
    ...
    if actual_estimated_cost is not None and actual_estimated_cost > max_budget_rub:
        return 3
    return 0 if report.qualification_status == "runtime_qualified" else 2
