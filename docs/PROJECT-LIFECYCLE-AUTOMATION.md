...
При запуске через Issue-label workflow публикует полный audit/apply result
обратно в эту Issue, включая ссылку на Actions run. Комментарий создаётся и
при fail-closed ошибке, чтобы конфликт не оставался скрытым только в логе.
