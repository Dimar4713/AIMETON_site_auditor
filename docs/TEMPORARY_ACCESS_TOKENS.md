...
Requires authenticated admin, existing CSRF protection, target `subject_user_id`, bounded expiry/TTL, `max_uses`, label, purpose and audit reason. Response returns metadata plus plaintext `token` once.
...
7. return standard `UserResponse` plus non-secret temporary-access metadata if useful.
...
Login gate supports two modes:

- **Пароль** — existing username/password flow;
- **Временный доступ** — paste token or consume an explicit invitation link.

For emailed campaigns the invitation should carry the token in a URL fragment, for example `/#access_token=...`, so ordinary server access logs do not receive the secret as a query parameter. Frontend must immediately remove the fragment from the address bar after exchange and never place the token in local/session storage.

A QR code may later encode the same invitation URL; it remains the same credential and is still subject to TTL/use limits.
...
- event/demo booth: separate campaign-specific token/user policy, short TTL and deliberately higher use limit; never reuse an individual's invitation credential.
...
- token successfully exchanged: token id, subject user, use counter, timestamp;
- token revoked: actor, reason, timestamp;
- exhausted/expired state can be derived without logging the presented token.
...
- marketing/demo token cannot grant ADMIN privilege;
...
- invitation URL does not leak token through query-string server logs/referrers;
