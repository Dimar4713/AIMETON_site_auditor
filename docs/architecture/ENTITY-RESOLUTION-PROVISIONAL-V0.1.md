...
Статус: provisional-срез `SA-SR-03 / #84` и Evidence Guard слиты PR `#99`
и `#101`, развёрнуты на stage. Текущая подтверждённая версия `0.16.0`,
merge/deployment SHA `1ed2376ab589b1174da29998cfbda7fdb4ede127`;
Deploy Stage `#30516485317` — success.
...
- наименование — Unicode-aware нормализация пробелов, кавычек и известных
  описательных хвостов title/H1;
- адрес — только после отдельной метки `адрес`, с границей слова и блока.
...
точному нормализованному значению. Внутри документа последовательные группы
сильных реквизитов связываются с ближайшим юридическим наименованием по
порядку provenance-bearing блоков. Равноудалённая привязка остаётся
`ambiguous_document_attribution`.

Юридическое наименование банка или иного платёжного контрагента без
собственного ближайшего ИНН/ОГРН сохраняется отдельным вторичным кандидатом.
Ему не наследуются телефон, email и адрес target-компании, и он не блокирует
сильного кандидата только фактом присутствия в том же документе.
...
`same_name_different_identifiers`. Два ближайших набора сильных реквизитов
двух субъектов также остаются competing; автоматическое «усреднение» запрещено.
...
revision history, inconsistent provenance, JSON Schema, настоящий HTTP-цикл
и обезличенные регрессии по структуре страниц Selectel, Sendy и БСК.
