...
- считает варианты русских/прямых кавычек одним наименованием;
- завершает quoted legal name на закрывающей кавычке, не включая хвост title/H1;
- извлекает пары `label → value` для разнесённых по блокам/ячейкам ИНН, ОГРН,
  наименования и адреса;
- требует границу слова и разделитель после метки `адрес`, поэтому `Адреса и
  контакты` не становится address signal.
...
Исключение ограничено банковскими/платёжными контрагентами: если документ
содержит ровно одно небанковское юридическое имя и одно или несколько
bank-like имён, сильные идентификаторы связываются только с единственным
небанковским target. Банк сохраняется отдельным competing subject без
target-реквизитов и без слабых сигналов основной компании. Если небанковских
имён два или сильные идентификаторы расходятся, автоматическое связывание
по-прежнему запрещено и создаётся conflict.
